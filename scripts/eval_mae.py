...
Produces:
  - A four-panel figure (table + MAE + Accuracy + AUC)
  - A standalone MAE figure

Usage:
    python eval_mae.py eval_mae_config.yaml
"""
...
def setup_logging(output_dir):
    ...
    file_handler = logging.FileHandler(output_dir / "eval_metrics.log")
...
def load_true_labels(data_csv, test_indices_file):
    ...
    return df.iloc[test_indices]["category_num"].to_numpy()


def load_predictions(models_dir, uuid):
    preds_file = pathlib.Path(models_dir) / f"{uuid}.preds.txt"
    return np.loadtxt(preds_file).astype(int)
...
def get_color(label):
    """Assign bar color from the project palette."""
    flat = label.replace("\n", " ")
    if "Always" in flat:
        return PALETTE["grey"]
    elif flat == "MLP":
...
def draw_bar_chart(ax, labels, values, selected_labels, *,
    ...
    ax.set_yticklabels(labels_sorted, fontsize=14)
    ...
    for i, (bar, val) in enumerate(zip(bars, values_sorted)):
        ...
        ax.text(bar.get_width() + max_val * 0.015,
                ...
                val_text, va="center", fontsize=12,
...
def main(config_file):
    ...
    for model in config["models"]:
        ...
        logger.info(f"{label:30s} (uuid={uuid}): MAE = {mae:.4f}, "
                     f"Accuracy = {acc:.4f}, AUC-OVO = {auc_ovo:.4f}")
    ...
    results_df.to_csv(output_dir / "metrics_results.csv", index=False)
    ...
    # ── Standalone MAE figure ─────────────────────────────────────────────
    fig_mae, ax_mae_solo = plt.subplots(figsize=(6, 8))
    draw_bar_chart(ax_mae_solo, all_labels, all_maes, selected_labels,
                   xlabel="Mean Absolute Error",
                   title="Test Set Mean Absolute Error",
                   fmt=".4f", higher_is_better=False)
    plt.tight_layout()
    fig_mae.savefig(output_dir / "mae_comparison.png", dpi=300,
                    bbox_inches="tight", facecolor="white")
    fig_mae.savefig(output_dir / "mae_comparison.svg",
                    bbox_inches="tight", facecolor="white")
    logger.info(f"MAE figure saved to {output_dir / 'mae_comparison.png'}")
    plt.close(fig_mae)
    ...
    for mp in model_props_list:
        label_display = mp["label"].replace("\n", " ")
        if mp.get("selected", False):
            label_display = f"{label_display}  *"
        table_data.append([
            label_display, mp["type"], mp["dca"], mp["library_bias"],
            mp["epochs"], mp["lr"], mp["wd"],
        ])
        row_selected.append(mp.get("selected", False))
        row_is_baseline.append(mp["type"] == "Baseline")

    table = ax_table.table(
        cellText=table_data, colLabels=table_headers,
    ...
    fig.savefig(output_dir / "metrics_comparison.png", dpi=300,
                bbox_inches="tight", facecolor="white", pad_inches=0.3)
    logger.info(f"Full figure saved to {output_dir / 'metrics_comparison.png'}")
    plt.close(fig)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python eval_mae.py eval_mae_config.yaml")
        sys.exit(1)
    main(sys.argv[1])
