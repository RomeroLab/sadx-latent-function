...
class SequentialWithArgs(nn.Sequential):
    def forward(self, x, **kwargs):
        for module in self:
            if isinstance(module, (ra.RelativeTransformerEncoder, SequentialWithArgs, mixout.Mixout)):
...
class TransferModel(nn.Module):
    ...
    def add_model_specific_args(parent_parser):
        ...
        p.add_argument("--use_mixout", action="store_true", default=False,
                       help="use mixout in the backbone")
        p.add_argument("--mixout_rate", type=float, default=0.7)
    ...
    def __init__(self,
        ...
        # apply mixout to the backbone if specified
        if use_mixout:
            # layers["backbone"].apply(lambda m: mixout.MixoutWrapper(m, mixout_rate))
            # layers["backbone"] = mixout.Mixout(layers["backbone"], mixout_rate)
            mixout.apply_mixout(layers["backbone"], mixout_rate)
