""" for fetching Rosetta data from the rosettafy variant database """
...
def convert_dms_to_rosettafy_indexing(ds_name, variants, reverse=False):
    ...
    if "rosettafy_pdb_offset" in constants.DATASETS[ds_name]:
        indexing_offset = constants.DATASETS[ds_name]["rosettafy_pdb_offset"]
...
def query_database(db_fn, pdb_fn, variants):

    # connect to the database
    engine = sqla.create_engine('sqlite:///{}'.format(db_fn))

    subquery = ",".join("?" * len(variants))
    query = "SELECT * FROM variant WHERE pdb_fn=? AND mutations in ({})".format(subquery)
    df = pd.read_sql_query(query, engine, params=(pdb_fn,) + tuple(variants))
...
def query_database_old(db_fn, pdb_fn, variants, chunk_size=150000):
...
def get_rosetta_ds(ds_name: str,
                   db_fn: str = "/Users/sg/PycharmProjects/rosettafy/variant_database/database.db",
    ...
    pdb_fn = constants.DATASETS[ds_name]["rosettafy_pdb_fn"]
    # rosetta_ds = query_database(db_fn, pdb_fn, variants_rosetta_compatible)
    rosetta_ds = query_database_old(db_fn, pdb_fn, variants_rosetta_compatible)
