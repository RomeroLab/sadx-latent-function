...
import shared_model
from shared_model import CosineWarmupScheduler, ConstantWarmupScheduler
...
class RosettaTask(pl.LightningModule):
    ...
    def _shared_step(self, data_batch, batch_idx, compute_loss=True):
        # todo: should this be done in forward() instead?
        ...
        # todo in the future, if we support multiple per batch, we can pass in all the pdb files
...
class SimpleInferenceTask(pl.LightningModule):
    # todo: reference TODOs from simple_inference.py
    @staticmethod
    def add_model_specific_args(parent_parser):
