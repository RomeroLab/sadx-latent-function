...
def plot_scatter(true_scores, predicted_scores, set_name, log_dir, fn_suffix=""):
    ...
    # joint_grid = sns.jointplot(x=true_scores, y=predicted_scores, **kwargs)
    ...
    # todo: instead of using min and max... remove outliers... because
    #   i was having a problem with extreme outliers messing up the scatterplot
    #   but also... why was i getting extreme outliers?
...
def save_predictions(raw_preds, dm, log_dir, save_format=("txt", "npy"), suffix=""):
...
class CosineWarmupScheduler(optim.lr_scheduler._LRScheduler):
    # courtesy of https://pytorch-lightning.readthedocs.io/en/latest/notebooks/course_UvA-DL/05-transformers-and-MH-attention.html
    # todo: might be able to make more modular implementation based on https://github.com/seominseok0429/pytorch-warmup-cosine-lr
    #   combine cosine annealing scheduler from PyTorch with a warmup wrapper
...
def fts_lambda(warmup_steps, transition_steps):
    """ learning rate lambda function for finetuning scheduler """

    def lr_lambda(current_step):

        # transition_steps is a list of steps at which to transition to the next phase
        # each phase has its own warmup and cosine decay
        # each phase can have a different total number of steps
        # in the future, we can make the last phase a constant LR or warmup+constant LR

        # find the current phase
        # default to the last phase
        phase = len(transition_steps) - 1
        for i, step in enumerate(transition_steps):
            if current_step < step:
                phase = i
                break

        # the start step for the current phase
        start_step = 0 if phase == 0 else transition_steps[phase - 1]

        # get the max steps for the current phase
        total_steps = transition_steps[phase]

        ws = warmup_steps
        if warmup_steps < 1:
            ws = int(warmup_steps * (total_steps - start_step))

        # cycle back to beginning if the current step exceeds total_steps
        # current_step = current_step % total_steps
        if current_step < (start_step + ws):
            return (current_step - start_step) / ws
        elif current_step < total_steps:
            progress = (current_step - start_step - ws) / (total_steps - start_step - ws)
...
def constant_warmup(warmup_steps):
    # https://pytorch-lightning.readthedocs.io/en/latest/notebooks/lightning_examples/barlow-twins.html?highlight=warmup#Learning-rate-warmup
...
def plot_losses(log_dir):
    ...
    # sns.move_legend(ax, "best", handles=handles,
...
def init_loggers(log_dir, my_uuid, wandb_online, wandb_project) -> list[WandbLogger, TensorBoardLogger, CSVLogger]:
...
class OptimizerConfig:
    ...
    # todo: set this up as multiple inheritance for tasks or as simple functions in a separate .py file
    # will make it less confusing for how it's meant to be used :)
    ...
    def __init__(self,
                 ...
                 max_epochs: int = None,
                 # for fts_lambda lr scheduler
                 fts_schedule_fn: str = None,
                 *args, **kwargs):
        ...
        self.max_epochs = max_epochs
        self.fts_schedule_fn = fts_schedule_fn
    ...
    def get_optimizer_config(self,
        ...
        elif self.lr_scheduler == "fts_lambda":
            # linear warmup with cosine decay LambdaLR for finetuning scheduler

            # steps per epoch
            steps_per_epoch = estimated_stepping_batches / self.max_epochs

            # load the finetuning schedule from self.fts_schedule
            fts_schedule = ScheduleImplMixin.load_yaml_schedule(Path(self.fts_schedule_fn))
            # get the transition epochs from the schedule
            transition_epochs = []
            for k, v in fts_schedule.items():
                transition_epochs.append(v["max_transition_epoch"])

            # convert the transition epochs to steps
            transition_steps = [transition_epoch * steps_per_epoch for transition_epoch in transition_epochs]

            lr_lambda = fts_lambda(self.warmup_steps, transition_steps)
