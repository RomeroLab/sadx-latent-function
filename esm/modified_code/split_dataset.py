...
def reduced_train_size(ds, val_size=.1, test_size=0., train_prop=.5, num_train_reps=5,
    ...
    """ create splits for reduced train size model. this function first removes any withholding. then it computes
        the val and test splits using the full amount of data remaining after removing the withholding. then from the
        remaining data, it generates the requested number of train replicates using the requested proportion.
        if specifying the same random seed, should result in the same val and test splits as train_test_split().
        further, if you specify a train prop of 1, this is equivalent to doing a regular train-val-test split where
        train_size = 1 - (val_size + test_size) """

    # set the random seed
    np.random.seed(rseed)

    # each replicate will still have the same val and test splits
    split_template = {}
    ...
    if test_size > 0:
        if isinstance(test_size, float):
            ...
            raise ValueError("unexpected type for test_size {}".format(test_size))
        idxs, test_idxs = train_test_split(idxs, test_size=adjusted_test_size)
    ...
    for i in range(num_train_reps):
        # make a copy of the template containing val, test, and withheld indices
        split_i = split_template.copy()
        if isinstance(train_prop, float) and train_prop == 1:
            # train proportion of 1 is a special case where all remaining indices not in val, test, or withheld
            # are placed in the test set. This is equivalent to doing a regular train-val-test split where
            # train_size = 1 - (val_size + test_size)
            split_i["train"] = idxs
    ...
    if out_dir is not None:
        ...
        out_dir_split = join(out_dir, "reduced_tr{}_tu{}_te{}_w{}_s{}_r{}".format(train_prop, val_size, test_size, w,
...
def resampled_dataset_size(
    ...
    """ newer version of reduced_train_size that takes the validation set from the reduced data instead of full data.
        this function first removes any withholding. then it computes the test split using the full amount of data
        remaining after removing the withholding. then from the remaining data, it generates requested number of
        dataset replicates using the requested dataset size. the dataset is split into train and validation based on
        the val_prop """
...
def load_standardization_params(split_dir, train_only=True):
    # todo: not sure if this function belongs in split_dataset, even though the standardization params are saved
    #  in the split directory, it's more of a data thing that might be better in source_model.py or datasets.py
    ...
    # if train_only:
    #     means_fn = join(standardization_params_dir, "energy_means_train.npy")
    #     stds_fn = join(standardization_params_dir, "energy_stds_train.npy")
    # else:
    #     means_fn = join(standardization_params_dir, "energy_means_all.npy")
    #     stds_fn = join(standardization_params_dir, "energy_stds_all.npy")

    # std_params = {"means": np.load(means_fn),
    #               "stds": np.load(stds_fn)}
