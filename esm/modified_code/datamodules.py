...
class DMSDataModule(pl.LightningDataModule):
    ...
    def __init__(self,
        ...
        self.aa_seq_len = len(constants.DATASETS[self.ds_name]["wt_aa"])
    ...
    def _init_pdb_fn(self, pdb_fn):
        if pdb_fn == "auto":
            self.pdb_fn = constants.DATASETS[self.ds_name]["pdb_fn"]
        else:
            self.pdb_fn = pdb_fn
    ...
    def get_variants(self, set_name: Optional[str]):
        if set_name is None:
            ...
            idxs = self.split_idxs[self.set_name_map[set_name]]
...
class RosettaDataModule(pl.LightningDataModule):
    ...
    ENERGY_NAMES = constants.ROSETTA_ATTRIBUTES
    ...
    def __init__(self,
        ...
        # database fn
        # todo: if i switch to split pdb_fns.txt, variants.txt, and HDF5 for energies, maybe specify full dir here?
        self.ds_fn = ds_fn

        # target/task names and number of tasks
        self.target_names = utils.get_rosetta_energy_names(target_names, target_names_exclude)
        self.num_tasks = len(self.target_names)
        ...
        # todo: implement better support for pdb_fns with full paths and different directories
        #   note i don't think we are going to support identical pdb base names with different directories
        #   because the pdb basename is used as a unique dictionary key in various places
        ...
        # todo: switch to PDB index for each dataset containing only the PDBs for that dataset
    ...
    def get_example_input_array(self):
        ...
        # todo: this must be compatible with the task's forward() method... it is.
    ...
    def get_ds(self, set_name: str) -> torch.utils.data.Dataset:
        # todo: if i end up loading the full pdb_fns list in the RosettaDatasetSQL into memory,
        #   I can grab it from there instead of also storing it in the datamodule
